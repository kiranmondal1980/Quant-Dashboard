...
with st.sidebar:
    ...
    dot_color   = "#00CC88" if is_open else "#FF4B4B"
    status_text = "LIVE · NSE" if is_open else "CLOSED · NSE"
    st.markdown(
        ...
        f'{status_text}</div>'
        f'<p style="font-size:0.66rem;color:rgba(128,128,128,0.65);margin-top:0.3rem">{ts}</p>',
        unsafe_allow_html=True,
    )
