...
# ─── PURE NUMPY/PANDAS INDICATORS (no pandas-ta / numba dependency) ──────────────
...
def build_chart(df: pd.DataFrame, pivots: dict, sig: dict) -> go.Figure:
    ...
    fig.update_layout(
        ...
        legend=dict(
            ...
            font=dict(size=10, color="#E0E0E0"), x=0.01, y=0.98,
        ),
    ...
    fig.update_xaxes(
        ...
        showspikes=True, spikecolor="#333C55", spikedash="dot", spikethickness=1,
    )
...
with st.sidebar:
    ...
    st.markdown(
        ...
        f'<span class="status-dot" style="background:{dot_color};box-shadow:0 0 6px {dot_color}"></span>'
        f'NSE {status_text}</div>'
            ...
            <div class="trade-log-entry">
              <div>
                ...
                <div class="trade-log-price">₹{entry['entry']:,.2f}
                  <span style="color:#555E78;font-size:0.7rem"> {sl_s} {tgt_s}</span>
                </div>
              </div>
